...
events
  ...
  event_start_time, event_end_time,   -- REQUIRED, explicit, LOCAL wall-clock = the event's PUBLIC start/end (audience / tickets / validation read these; authoritative value, always machine-findable regardless of label). #16.4 (RESOLVED) — supersedes #10's "no fixed columns" for these two.
  -- Other named times (door/soundcheck/curfew/…) are modular schedule_items (#10). The two public times each BIND to a schedule item whose label is editable-but-revertible (nullable custom_label; NULL = default label) — the time stays canonical. Default schedule seeded from the old app: Get in → Soundcheck → Doors open → Support Act → Show start → Curfew → Clear venue.
  ...
  age_restriction, event_type, mood_style_tags[],   -- 2026-07-24 (#16.6): age/event-type + controlled mood/style tags; accommodation + artist logistics per #16.7; genre/promoter NOT inline (profile hover-preview)
...
deals                         -- ALSO holds the agreement (agreement folded in, 1:1 — decision 2026-07)
  id, event_id → events,
  type,                       -- performance | rental | fee | split  ('custom' REMOVED 2026-07-24, decisions #16.2 — free text broke the engine; uncovered arrangements = notes on a NULL-structure deal + a "deal type not listed?" contact button)
...
**AI, territory & onboarding (2026-07-24 — DESIGN BEFORE BUILD; full detail: `docs/decisions.md` #16):**
- **AI / assistant layer** — a Gemini in-app assistant + **agent-native** (bring-your-own-AI) surface, both built on
  **the `authorize(capability)` catalog exposed as tools**. Build the manual routes tool-shaped so this is a thin
  add-on. **`agent` is reserved for the account kind** — the assistant is `assistant`/`ai` (#16.14–16.15).
- **Territory-based architecture (RESOLVED, decisions #17)** — the boundary is **derived from location, not stamped**:
  `country` on every account/profile/event (tax/PRO/currency) + a configurable **`market` = grouping of countries**
  (`country → market` mapping; may be one country, a region like EU, or worldwide), enforced **softly in
  `authorize()`/`serialize()` (one DB)**. Re-drawable country→region→city with no data migration; keep the finest
  location (`city`+`lat/lng`) on every row. Physical per-market instances = later promotion path only.
- **Onboarding + ingestion** — TypeForm-style flow, pre-built venue profiles, Bolagsverket/Google enrichment, VAT
  capture, Spotify/Facebook artist verification, AI event-draft ingestion (scrape/PDF/Excel/poster) + Google-Workspace
  contact import & merge (#16.16–16.17).
- **Financial analytics page** — a dedicated paid analytics surface (planned vs projected vs concluded), fed by a
  **budget snapshot captured at settlement** (#16.8–16.9).
