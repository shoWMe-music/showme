# CLAUDE.md — shoWMe project context

This project is **shoWMe**, a pre-launch live-events booking + settlement SaaS.

**Files in this project:**
- `uploads/shoWMe App.html` — the exported prototype bundle (reference).
- `Prototype/shoWMe Prototype.dc.html` — the prototype reconstructed as an editable interactive Design Component (source of truth for UI iteration). Fonts live in `Prototype/`; React loads from unpkg CDN.
- `shoWMe Design System.dc.html` — design-system reference (tokens, type, components) derived from the product.

**User types (account kinds):** operator | performer | professional(crew). See the plan for how kind gates surfaces vs. how ReBAC gates permissions.

**When working here:** the prototype is the visual/UX reference; the blueprint below governs the backend rebuild (new Postgres project, separate repo). Keep UI edits aligned with the design system and the account-kind / party-scoped-visibility rules in the plan.

**UI conventions (memory):** icon-only buttons — modal close (×), back arrows, and similar — have NO background and NO border (`background:transparent;border:0;`). Just the glyph in a muted color.
...
- **Product layer = account kind:** a user account has a **kind** (operator | performer | crew) chosen at signup; it gates profile creation, dashboard, features, and pricing. Kept **separate** from authorization (entitlement ≠ permission).
...
event_participants           -- replaces participants + collaborators subcollections,
  ...
  profile_id → profiles,
  role,                       -- host | co_host | performer | support | crew_lead | crew
...
```
settlements(id, event_id, participant_id → event_participants, status, computed jsonb,
            manual_overrides jsonb, created_at, updated_at)          -- ONE per participant profile
...
2. **Entitlement `E_p`** per party from their deal (ported `calculateSettlement` primitives): fixed guarantee/rental ·
   ...
   credit agent) · bonus if threshold · costs assigned to a deal reduce that party's `E` · **operator = residual**
...
users(id = firebase uid, email, name, initials, avatar_url, currency, date_format,
      time_format, timezone, company_name, country, kind[operator|performer|professional],
...
```
riders(id, event_id → events, owner_participant_id → event_participants, type, name,
       description, file_url, file_name, created_by, timestamps)
