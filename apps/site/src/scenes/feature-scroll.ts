...
/* Feature scroll — pinned swap, with a role picker.
   Pick who you are (Operator / Performer / Agent / Professional) and the products
...
export function initFeatureScroll() {
  ...
  // Phones fall back to the same static stacked list as reduced-motion (the pinned
  // swap is unreadable/fragile at phone width) — see the ≤600px CSS above.
  const mob = window.matchMedia('(max-width: 600px)').matches;
  const staticLayout = reduce || mob;

  // Each role, with a one-line explanation of what that account kind is — so a
  // visitor knows whether they're an operator, performer, agent or professional.
  const KINDS = [
    ...
    { key: 'professional', label: 'Professional', desc: 'Freelance crew and services — sound, lighting, stage, security — paid a fee to make the event happen.' },
  ];
  ...
  const SETS = {
    operator: [
      ...
      { name: 'Availability', t: 'Publish when you’re open. Hold what you’re deciding.', d: 'Share your open dates with the collaborators you choose. Ranked holds promote automatically when one drops.', img: 'light-02-availability' },
      ...
      { name: 'Team', t: 'Your team, your crew, and everything to do.', d: 'Bring your people — each with access to exactly what they need. Reusable groups drop a whole crew on in one action.', img: 'light-06-team' },
    ...
    performer: [
      { name: 'Availability', t: 'Publish your open dates and tour windows.', d: 'Share when you’re free with the promoters and venues you choose — and let competing holds rank themselves.', img: 'light-02-availability' },
      { name: 'Offers', t: 'Every offer in one inbox.', d: 'Bookings and offers land in one structured place — the date, the room and the fee at a glance. Reply, counter or pass.', img: 'light-01-requests' },
      ...
      { name: 'Agreement', t: 'Your deal, confirmed and stored.', d: 'The terms you agreed become the record — fee, split, date — and you confirm your own side. No lost PDFs.', img: 'light-05-agreement' },
    ...
    agent: [
      ...
      { name: 'Offers', t: 'Every offer, for every act.', d: 'Incoming offers for all your performers in one inbox — sorted by act, date and fee. Move on the good ones fast.', img: 'light-01-requests' },
    ...
    professional: [
      { name: 'Availability', t: 'Publish when you’re available.', d: 'Share the dates you can work with the operators and crews who book you — and mark what’s already held.', img: 'light-02-availability' },
  ...
  style.textContent = `
    ...
    /* Phones: a 720vh sticky-pinned swap is fragile and hard to read on a small
       screen. Unpin into a simple stacked list of the selected role's features
       (same shape as the reduced-motion fallback). The scene bails its scroll
       loop at this width (see below) so these static styles hold. */
    @media (max-width: 600px) {
      .feat-scroll { height: auto; }
      .feat-pin { position: static; height: auto; display: block; overflow: visible; padding: 40px 0 20px; }
      .feat-head { position: static; left: auto; right: auto; margin: 0 auto 6px; padding: 0 24px; }
      .feat-roledesc { display: none; }
      .feat-stage { height: auto; display: block; width: min(100% - 40px, 640px); margin: 0 auto; }
      .feat-texts { position: static; display: flex; flex-direction: column; gap: 44px; margin-top: 24px; }
      ...
      .feat-tabs { gap: 7px; }
      .feat-tab { padding: 7px 14px; font-size: 12.5px; }
    }
  ...
  function selectKind(kind) {
    ...
    if (staticLayout) { buildStage(kind); return; }
  ...
  if (staticLayout) return; // CSS shows a plain stacked list of the default role's copy
