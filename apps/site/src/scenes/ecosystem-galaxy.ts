...
export function initEcosystemGalaxy() {
  ...
  function resize() {
    ...
    mobile = W < 640;
    unit = Math.min(W * (mobile ? 0.64 : 0.86), H * 1.02);
  }
  resize();
  ...
  host.addEventListener('mousemove', (e) => {
    ...
    // Very gentle parallax — only a slight lean, never a big swing.
    targetTiltY = ((e.clientX - r.left) / r.width - 0.5) * 0.26;
    targetTiltX = ((e.clientY - r.top) / r.height - 0.5) * 0.18;
  });
  ...
  RINGS.forEach((ring, ri) => {
    ring.nodes.forEach((label, i) => {
      allNodes.push({
        ring: ri, label,
        angle: (i / ring.nodes.length) * Math.PI * 2 + ri * 1.25 + 0.6,
      });
    });
  });
