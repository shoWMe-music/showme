...
/**
 * Create the calendar this application owns in the user's account.
 *
 * ── WHY THIS DOES NOT LOOK FIRST ─────────────────────────────────────────────
 * The obvious shape is find-or-create: list `users/me/calendarList`, reuse a
 * calendar named "shoWMe" if it is there, create one otherwise. That is what this
 * function did until it was checked against Google's reference, and it was
 * broken: **`calendarList.list` is not authorized under `calendar.app.created`**
 * (it needs `calendar.readonly`, `calendar`, or one of the `calendarlist`
 * scopes). The listing would have answered 403 on every single call.
 *
 * Worse than failing: `googleWrite` maps a 403 to `GoogleAuthorizationRevokedError`,
 * and `syncCalendarConnection` reacts to that by marking the connection as needing
 * reconnection. So the first push would have told every user their working Google
 * connection had been revoked, and the cause would have looked like an auth
 * problem rather than a wrong scope on one lookup.
 *
 * The id is therefore REMEMBERED, not rediscovered — `calendar_connections.app_calendar_id`
 * (migration 0034) is the only handle we have, which is exactly why that column
 * exists. `calendars.insert` IS authorized under `calendar.app.created`, so
 * creating is the one calendar-level operation this scope allows us.
 */
export async function createAppCalendar(input: {
  accessToken: string;
  summary: string;
  timeZone?: string | null;
  fetchImplementation?: typeof fetch;
