...
@media print{body{padding:20px}}
  ...
  html += `<p class="meta">${event.artist} · ${event.venue} · ${event.date} · Currency: ${currency} (${sym})</p>`;
  ...
  if (includeSection("event-info", "details")) {
    html += `<h2>Event Information</h2><table>
      ...
      <tr><td>Performer</td><td>${event.artist}</td></tr>
      <tr><td>Operator</td><td>${event.operator}</td></tr>
      <tr><td>Operator Type</td><td>${event.operatorType}</td></tr>
      <tr><td>Status</td><td>${event.eventStatus}</td></tr>
      <tr><td>Capacity</td><td>${event.capacity}</td></tr>
      <tr><td>Ticketing Provider</td><td>${event.ticketingProvider || "N/A"}</td></tr>
    </table>`;
  }
  ...
  if (includeSection("deal-structure", "details")) {
    html += `<h2>Financial Deal</h2><table>
      <tr><th>Field</th><th>Value</th></tr>
      <tr><td>Deal Type</td><td>${deal.dealType}</td></tr>`;
    if (deal.artistGuarantee) html += `<tr><td>Artist Guarantee</td><td>${formatCurrency(deal.artistGuarantee, currency)}</td></tr>`;
    if (deal.venueRental) html += `<tr><td>Venue Rental</td><td>${formatCurrency(deal.venueRental, currency)}</td></tr>`;
    if (deal.dealType !== "guarantee" && deal.dealType !== "rental" && (deal.artistSplit || deal.venueSplit)) html += `<tr><td>Split</td><td>Performer ${deal.artistSplit}% / Venue ${deal.venueSplit}%</td></tr>`;
    if (deal.artistCostSplit > 0 || deal.promoterCostSplit > 0 || deal.venueCostSplit > 0) html += `<tr><td>Costs Split</td><td>Performer ${deal.artistCostSplit || 0}% / Promoter ${deal.promoterCostSplit}% / Venue ${deal.venueCostSplit}%</td></tr>`;
    html += `</table>`;
  }

  // schedule and riders are stored in subcollections — not on eventMeta directly
  const scheduleItems = (md as unknown as { schedule?: ScheduleItem[] }).schedule;
  if (includeSection("production-schedule", "details") && scheduleItems?.length) {
    ...
    scheduleItems.forEach((s: ScheduleItem) => {
      html += `<tr><td>${s.time || ""}</td><td>${s.label}</td></tr>`;
    });
    html += `</table>`;
  }

  const riderItems = (md as unknown as { riders?: Rider[] }).riders;
  if (includeSection("riders", "details") && riderItems?.length) {
    html += `<h2>Riders & Documents</h2><table>
      <tr><th>Type</th><th>Name</th></tr>`;
    riderItems.forEach((r: Rider) => {
      html += `<tr><td>${r.type}</td><td>${r.name}</td></tr>`;
    });
    html += `</table>`;
  }

  const agreementItems = (md as unknown as { agreements?: Agreement[] }).agreements;
  if (includeSection("agreements-docs", "agreement") && agreementItems?.length) {
    ...
    agreementItems.forEach((a: Agreement) => {
      html += `<tr><td>${a.type}</td><td>${a.name}</td><td>${a.status}</td></tr>`;
    });
    html += `</table>`;
  }
  ...
  const crewItems = (md as unknown as { crew?: CrewMember[] }).crew;
  if (includeSection("shared-team", "crew") && crewItems?.length) {
    ...
    crewItems.forEach((c: CrewMember) => {
      html += `<tr><td>${c.name}</td><td>${c.role}</td><td>${c.email || ""}</td><td>${c.phone || ""}</td><td>${c.collaborator || ""}</td></tr>`;
    });
    html += `</table>`;
  }
